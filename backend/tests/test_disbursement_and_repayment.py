"""CLAUDE.md M7: simulated disbursement & repayment.

Disbursement is credit-officer-only, compare-and-set (§14), writes a ledger
Transaction + audit entry, and is blocked while the company is suspended
(§6: "new applications / disbursements: stopped"). Repayment is customer-only,
also compare-and-set, and stays on the suspension allow-list (rule #10: never
trap a borrower with a debt they owe).
"""
...
def test_pending_disbursement_queue_lists_approved_loans_and_clears_after_disbursement(client, engine):
    ...
    queue = client.get("/credit/loans/pending-disbursement", headers=_auth_headers(ctx["credit_token"]))
    ...
    queue_after = client.get("/credit/loans/pending-disbursement", headers=_auth_headers(ctx["credit_token"]))
...
def test_disburse_moves_loan_to_active_and_writes_transaction_and_audit(client, engine):
    ...
    resp = client.post(f"/credit/loans/{loan_id}/disburse", headers=_auth_headers(ctx["credit_token"]))
    assert resp.status_code == 200, resp.text
    body = resp.json()
...
def test_double_disburse_is_a_conflict(client, engine):
    ...
    first = client.post(f"/credit/loans/{loan_id}/disburse", headers=_auth_headers(ctx["credit_token"]))
    assert first.status_code == 200

    second = client.post(f"/credit/loans/{loan_id}/disburse", headers=_auth_headers(ctx["credit_token"]))
...
def test_non_credit_officer_cannot_disburse(client, engine):
    """As of M10 (§3), `compliance_token` in this fixture is itself a
    credit_officer (compliance_officer retired) and so may legitimately
    disburse today — moving disbursement to cashier_finance_officer is M14
    (§28), not yet built. Only system_administrator and customer are
    rejected here."""
    ctx = _setup_approved_loan(client, engine)
    loan_id = ctx["loan"]["id"]

    for token in (ctx["admin_token"], ctx["customer_token"]):
        resp = client.post(f"/credit/loans/{loan_id}/disburse", headers=_auth_headers(token))
        assert resp.status_code == 403
